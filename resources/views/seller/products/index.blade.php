    ...
    <div class="card overflow-hidden">
        ...
        @else
          <form method="POST" action="{{ route('seller.products.bulk-stock') }}">
                            ...
                            <tr class="hover:bg-dark-50/30 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        <button type="submit" form="delete-form-{{ $product->id }}" onclick="return confirm('Hapus produk ini?')" class="p-2 rounded-lg text-dark-500 hover:bg-red-50 hover:text-red-600 transition-colors" title="Hapus">
            ...
            @foreach ($products as $product)
                <form id="delete-form-{{ $product->id }}" method="POST" action="{{ route('seller.products.destroy', $product) }}" style="display: none;">
                    @csrf
                    @method('DELETE')
                </form>
            @endforeach
